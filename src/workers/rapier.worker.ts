...
export async function evaluateMandibularSweep(request: Extract<PhysicsWorkerRequest,{type:"evaluate-mandibular-sweep"}>): Promise<SweepResult> {
 const frames=[];
 for(const {frameIndex,progress,pose:requestedPose} of sweepPoses(request.preset,request.frameCount)){const pose=await evaluateMandibularPose({id:request.id,type:"evaluate-mandibular-pose",fixtureId:request.fixtureId,sequence:request.sequence,pose:requestedPose});frames.push({frameIndex,progress,requestedPose:pose.requestedPose,appliedTransform:pose.appliedTransform,classification:pose.classification,measurementStatus:pose.measurementStatus,clearanceMeters:pose.clearanceMeters,penetrationDepthMeters:pose.penetrationDepthMeters,contactCount:pose.contactCount,contactSamples:pose.contactSamples});}
 return {fixtureId:request.fixtureId,sequence:request.sequence,preset:request.preset,frameCount:request.frameCount,finalPose:frames.at(-1)!.requestedPose,frames,summary:summarizeSweep(frames)};
}

export async function handleMessage(message: PhysicsWorkerRequest): Promise<PhysicsWorkerResponse> { const requestId = message.id; try { if (message.type === "health-check") { await ensureRapier(); return { id: requestId, type: "health", ok: true, protocolVersion: WORKER_PROTOCOL_VERSION, rapierVersion: "@dimforge/rapier3d-compat", fixtureName: SYNTHETIC_COLLISION_FIXTURE_NAME }; } if (message.type === "run-synthetic-collision-fixture") return { id: requestId, type: "synthetic-collision-result", ok: true, ...(await runSyntheticCollisionFixture()) }; if (message.type === "run-phase1-contact-query") return { id: requestId, type: "phase1-contact-result", ok: true, ...(await runPhase1ContactQuery(message)) }; if(message.type==="initialize-occlusion-fixture"){await initializeOcclusionFixture(message);return{id:requestId,type:"fixture-ready",ok:true,fixtureId:message.fixtureId,protocolVersion:WORKER_PROTOCOL_VERSION};} if(message.type==="evaluate-mandibular-pose") return{id:requestId,type:"mandibular-pose-result",ok:true,...await evaluateMandibularPose(message)}; if(message.type==="evaluate-mandibular-sweep") return{id:requestId,type:"mandibular-sweep-result",ok:true,...await evaluateMandibularSweep(message)}; const exhaustive: never = message; return { id: requestId, type: "error", ok: false, code:"invalid-request", message: `Unsupported physics worker request: ${JSON.stringify(exhaustive)}` }; } catch (error) { const message=error instanceof Error?error.message:"Unknown worker error"; const code=message==="not-initialized"||message==="fixture-mismatch"?message:"worker-error"; return { id: requestId, type: "error", ok: false, code, message: code==="not-initialized"?"Initialize the synthetic fixture before evaluating a pose.":code==="fixture-mismatch"?"The pose fixture does not match the initialized fixture.":message }; } }
