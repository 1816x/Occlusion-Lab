...
## Phase 4.1 parity boundary

The first migration boundary is a versioned golden pose fixture. A golden fixture is deterministic, tracked JSON generated from the pure legacy TypeScript reference and consumed independently by TypeScript and native tests. `occlusion-core` owns strongly typed pose limits, validation, and a pure pose-to-`RigidTransform` operation; it remains independent of JSON and filesystems. The native test target alone uses pinned nlohmann-json 3.11.3 and receives a configured copy of the fixture, so tests do not depend on their launch directory.

Parity is deliberately limited to meters/millimeters, inclusive pose limits, finite acceptance/rejection and fields, legacy translation (`x = lateral`, `y = 0.16 - opening`, `z = protrusion`), and identity rotation, compared at `1e-12` meters. It does not cover Rapier results, collision, contacts, clearance, penetration, sweeps, rendering, or clinical behavior. Rapier and the authoritative production evaluation remain in the Worker; React and the main thread do not calculate transforms or physics. Protocol version 4 is unchanged.

## Phase 4.2 sweep-generation boundary

The pure TypeScript sweep reference is dependency-free and remains shared by the production Worker. It selects `closing` (neutral to contact), `protrusive` (contact to maximum +Z), `left-lateral` (contact to minimum signed X), and `right-lateral` (contact to maximum signed X) endpoints. It accepts 2–61 frames (31 default), calculates unrounded progress as `index / (count - 1)`, linearly interpolates each component, applies six-decimal pose quantization, and preserves exact endpoint frames.
